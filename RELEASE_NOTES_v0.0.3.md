...
### Numbered HDMI Icons
HDMI sources now show individually numbered icons in the media player card:
- **HDMI1** → `hdmi1.svg` (with "1" overlay)
- **HDMI2** → `hdmi2.svg` (with "2" overlay)
- **HDMI3** → `hdmi3.svg` (with "3" overlay)

This makes it easy to identify which HDMI input is active at a glance.

## Previous Changes (since v0.0.1)

### Source Icons in Media Player Card
The media player card shows a visual icon for the currently active video source. Icons are automatically copied to your Home Assistant `www/acerprojector/` folder on first setup.

### Volume Control in Media Player
The media player entity supports volume control:
...
| Commit | Description |
|--------|-------------|
| `723be05` | feat: numbered HDMI icons (hdmi1, hdmi2, hdmi3) with text overlay |
